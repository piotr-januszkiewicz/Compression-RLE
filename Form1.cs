...
namespace Ja_proj_cs
{
    public partial class kompresja : Form
    {
        ...
        private void Button1_Click(object sender, EventArgs e)
        {
            ...
            sizeBeforeCompression = compression.sizeOfByteArray(bmp).Length;
            Size.Text = sizeBeforeCompression.ToString();

            stopwatch.Start();
            if (isAssembler == 0)
            {
                compression.startCompression(bmp, threads);
                sizeAfterCompression = compression.sizeOfCompressedFile();
                Size1.Text = sizeAfterCompression.ToString();
                AfterCompression.Image = compression.convertArrayToBitmap();
            } else
            {
                int size = bmp.Width * bmp.Height * 3;
                int[] arrayOfColorIngredients = new int[size];
                arrayOfColorIngredients = compression.convertToIntArray(bmp);

                compression.startCompressionAssembler(bmp, arrayOfColorIngredients, threads);
                sizeAfterCompression = compression.sizeOfCompressedFile();
